...
import com.furkanbesirli.foodencyapp.databinding.SearchRvBinding
import com.furkanbesirli.foodencyapp.utils.FOLLOW
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Firebase.firestore.collection(Firebase.auth.currentUser!!.uid+ FOLLOW).whereEqualTo("email",userList.get(position).email).get().addOnSuccessListener {
            ...
            if (it.documents.size==0){
                ...
                holder.binding.follow.text="Takibi Bırak!"
                isFollow=true
            }
        ...
        holder.binding.follow.setOnClickListener {
            if(isFollow){
                Firebase.firestore.collection(Firebase.auth.currentUser!!.uid+ FOLLOW).whereEqualTo("email",userList.get(position).email).get().addOnSuccessListener {
                    ...
                    holder.binding.follow.text="Takip Et"
                    isFollow=false
